...
function filterByAwardxCountry(award,country) {
  var awardData =filterByAward(award);
...
function byTeam(team){
  ...
  for(let i=0; i<players.length; i++){
    if(players[i].team ==team){
      fPlayer[count++] = players[i];
    }
  }
  for(let i=0; i<fPlayer.length; i++){
    for(let j = i+1; j<fPlayer.length; j++){
      if(fPlayer[i].awards.length<fPlayer[j].awards.length){
        let temp = fPlayer[i];
        fPlayer[i] = fPlayer[j];
        fPlayer[j] = temp;
      }
...
function sortName(award,noOfTimes,country){
  var dataOne = filterByAwardxTimes(award,noOfTimes);
